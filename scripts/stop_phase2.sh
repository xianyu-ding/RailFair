#!/bin/bash
# Stop Phase 2 background batch collection process
# Usage: ./stop_phase2.sh [pid_file]
...
echo "🛑 Stopping Phase 2 process $PID..."
...
for i in {1..30}; do
    if ! ps -p "$PID" > /dev/null 2>&1; then
        ...
        echo "   Log: $LOG_FILE"
        echo "   Progress: data/progress_phase2.json"
        exit 0
    fi
    sleep 1
    if [ $((i % 5)) -eq 0 ]; then
...
if ps -p "$PID" > /dev/null 2>&1; then
    ...
    echo "   Check progress file: data/progress_phase2.json"
fi
