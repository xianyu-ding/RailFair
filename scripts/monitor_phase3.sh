#!/bin/bash
# Monitor Phase 3 collection progress in real-time
# Usage: ./monitor_phase3.sh [refresh_interval_seconds]
...
if [ -z "$PID_FILE" ] || [ ! -f "$PID_FILE" ]; then
    echo "❌ No Phase 3 process found. Start with: ./run_phase3_background.sh"
    exit 1
fi
...
PROGRESS_FILE="${DATA_DIR}/progress_phase3.json"
DB_FILE="${DATA_DIR}/railfair.db"

echo "📊 Phase 3 Collection Monitor (Refresh: ${REFRESH_INTERVAL}s)"
...
while true; do
    clear
    echo "═══════════════════════════════════════════════════════════"
    echo "📊 Phase 3 Batch Collection Monitor"
    echo "═══════════════════════════════════════════════════════════"
    echo ""
