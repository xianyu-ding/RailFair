#!/bin/bash
# Stop Phase 1 background batch collection process
# Usage: ./stop_phase1.sh [pid_file]
...
if [ -n "$1" ]; then
    ...
    PID_FILE=$(ls -t "$LOG_DIR"/phase1_batch_*.pid 2>/dev/null | head -1)
fi

if [ -z "$PID_FILE" ] || [ ! -f "$PID_FILE" ]; then
    echo "❌ No Phase 1 PID file found. Process may not be running."
    exit 1
fi

PID=$(cat "$PID_FILE")
TIMESTAMP=$(basename "$PID_FILE" .pid | sed 's/phase1_batch_//')
...
echo "🛑 Stopping Phase 1 process $PID..."
...
for i in {1..30}; do
    if ! ps -p "$PID" > /dev/null 2>&1; then
        ...
        echo "   Log: $LOG_FILE"
        echo "   Progress: data/progress_phase1.json"
        exit 0
    fi
    sleep 1
    if [ $((i % 5)) -eq 0 ]; then
...
if ps -p "$PID" > /dev/null 2>&1; then
    ...
    echo "   Check progress file: data/progress_phase1.json"
fi
